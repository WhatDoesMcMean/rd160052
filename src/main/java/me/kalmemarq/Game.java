...
public class Game implements Runnable, Window.EventHandler {
    ...
    private static final String VERSION = "rd160052";
    ...
    private void renderHud() {
        ...
        int crosshairWidth = 16;
        int crosshairHeight = 16;

        tessellator.begin(DrawMode.LINES, VertexLayout.POS);
        builder.vertex((width / 2f), (height / 2f - crosshairHeight / 2f), 0);
        builder.vertex((width / 2f), (height / 2f + crosshairHeight / 2f + 1f), 0);

        builder.vertex((width / 2f - crosshairWidth / 2f), (height / 2f), 0);
        builder.vertex((width / 2f + crosshairWidth / 2f), (height / 2f), 0);
        tessellator.draw();
    }
